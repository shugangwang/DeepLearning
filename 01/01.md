...
批量大小和学习率是人为设定的，并不是模型训练出来的，因此叫做**超参数**（hyperparameter）。通常所说的**调参**就是调节超参数。

**6、模型预测**

设模型参数 <img src="http://latex.codecogs.com/gif.latex?w_1, w_2, b"/> 优化后的值分别为 <img src="http://latex.codecogs.com/gif.latex?w_1^*, w_2^*, b^*"/>，

### &sect;1.1.2 线性回归的表示方法

**1、神经网络图**

神经网络中，输入个数叫做**特征数**或**特征向量维度**。线性回归是一个单层神经网络。输出层是**全连接层**（fully-connected layer）或**稠密层**（dense layer）。
